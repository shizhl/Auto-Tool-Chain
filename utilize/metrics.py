...
import random
from .apis import get_from_openai


__all__ = ['eval_f1', 'f1_score', 'eval_rouge', 'eval_bleu', 'eval_meteor', 'eval_distinct', 'eval_all',
           'eval_acc', 'eval_em','eval_cover_em','eval_bert_score','eval_recall_prec','normalize','is_cover','can_convert_to_num']
...
def normalize(s):
    ee = ['"', "'", ',', '*', '#', 'to ', 'until', ' to', 'from', 'the']
    for e in ee:
        s = s.replace(e, ' ')
    ee = ['-']
    for e in ee:
        s = s.replace(e, '')
    s = ' '.join([e for e in s.split(' ') if e != '' and e.isascii()])
    return s
...
def is_cover(s1, s2):
    ...
    if s1.lower() in s2.lower() or s2.lower() in s1.lower():
        return True
    # if can_convert_to_num(s1):
    #     s1 = str(w2n.word_to_num(s1))
    # if can_convert_to_num(s2):
    #     s2 = str(w2n.word_to_num(s2))
    s1 = s1.lower().replace('.', ' ').split(' ')
    s2 = s2.lower().replace('.', ' ').split(' ')
    over = len([e for e in s1 if e in s2])
    if len(s1) == over or len(s2) == over:
        return True
    return False

################## cover-em ###############
def eval_cover_em(predictions, raw):
    ...
    for i in range(len(_predictions)):
        for _r in _raw[i]:
            if is_cover(_predictions[i],_r):
                _predictions[i] = _r
    c_em=eval_em(_predictions,_raw)['NQ-EM']
    return {'Cover-NQ-EM':c_em}

def _LLM_ACC(query,output,answer,model='text-davinci-003'):

    prompt="""In the following task, you are given a Question, a model Prediction for the Question, and a Ground-truth Answer to the Question. You should decide whether the model Prediction implies the Ground-truth Answer.
Question: {query}
Prediction: {output}
Ground-truth Answer: {answer}

Does the Prediction imply the Ground-truth Answer? Just Output Yes or No.""".format(query=query,answer=answer,output=output)
    ans=get_from_openai(model=model,prompt=prompt,tt='text')
    return ans.strip()


def eval_LLM_acc(queries,predictions,references,model='gpt-3.5-turbo-instruct'):
    result=[]
    for query, output, answer in zip(queries,predictions,references):
        result.append(
            _LLM_ACC(query,output,answer,model=model)
        )
    result= [1 if 'yes' in e.lower() else 0 for e in result]
    return sum(result)/len(result)

def eval_bert_score(predictions,references):
    from evaluate import load
    bertscore = load("bertscore")
    results = bertscore.compute(predictions=predictions, references=references, lang="en")
    results={
        "bs-p":sum(results['precision'])/len(results['precision']),
        "bs-r":sum(results['recall'])/len(results['recall']),
    }
    results['bs-f1']=2*results['bs-p']*results['bs-r']/(results['bs-p']+results['bs-r'])
    return results
...
def eval_em(predictions, raw):
    ...
    for pred, example in zip(predictions, raw):
        pred = norm_text(pred)
        if any([norm_text(ans) in pred for ans in example]):
            scores.append(1)
        else:
            scores.append(0)
...
def eval_recall_prec(predicts,answers):
    recall,precise,f1_score=[],[],[]
    for pred,ans in zip(predicts,answers):
        pred=normalize_answer(pred).split()
        pred=[e for e in pred if len(e)>=3]
        ans=normalize_answer(ans).split()
        ans=[e for e in ans if len(e)>=3]
        p,r,f1  =  _prec_recall_f1_score(pred,ans)
        precise.append(p)
        recall.append(r)
        f1_score.append(f1)
    return  sum(precise)/len(precise),sum(recall)/len(recall), sum(f1_score)/len(f1_score)
